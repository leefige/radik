#ifndef TOPK_RADIXSELECT_UTILS_CUH_
#define TOPK_RADIXSELECT_UTILS_CUH_
...
#ifdef ENABLE_BF16
#include "bf16/hie_bfloat16.hpp"
#include "bf16/hie_bfloat16_cmath.hpp"
#endif
...
struct alignas(sizeof(T) * N) VT {
    ...
    __device__ __forceinline__ T &operator[](const int &idx) {
        return data[idx];
    }
...
__device__ __forceinline__ typename ComputeT<VT<T, N>>::type scaling(
    ...
    #pragma unroll
    for (int i = 0; i < N; ++i) {
        val[i] = isNaN(val[i]) ? (LARGEST ? getMin<CompT>() : getMax<CompT>())
                               : val[i];
    }
#endif
    return WITHSCALE ? getScale(val, scaler) : val;
...
#endif  // TOPK_RADIXSELECT_UTILS_CUH_
